...
output:
  ...
  html_document:
    df_print: paged
subtitle: "Exploratory Data Analysis and Data Transformations"
---
...
To ensure the integrity of diagnostics, I will exclude plans with zero values in key count and contribution fields, specifically participant counts, participant contributions, employer contributions, loans, and total contributions. These exclusions are not arbitrary; they reflect a principled modeling decision grounded in interpretability.

Plans reporting zero contributions are structurally distinct. They may represent inactive accounts, legacy plans, or data artifacts that do not reflect participant engagement or plan behavior. Including them would dilute the analytic signal, obscure meaningful variation, and risk misrepresenting adequacy drivers.

By filtering out these zero-value plans, I preserve the validity of my comparisons, sharpen the diagnostic lens, and ensure that adequacy tiers reflect real behavioral differences, not suppressed or stagnant inputs. This approach supports ethical modeling, stakeholder clarity, and fairness-aware benchmarking across sectors.
...
The sharp uptick in the late 1970s through the 1990s coincides with major policy shifts, most notably the passage of ERISA in 1974 and the rise of defined contribution plans like 401(k) plans in the early 1980s. This era marks a surge in plan formation, especially among small and mid-sized employers responding to new regulatory clarity and tax incentives.
...
# Plan Type Name (PLAN_TYPE_NAME)

Extract the relevant codes from the plan type string and use them to create a new plan type name field.
...
This confirms the corporate sector’s pull in retirement plan design; 88% of the plans are of the 401(k) type. The remaining 12% is split across three structurally distinct 403(b) types.
...
Saturated sectors like Manufacturing and Professional Services are likely have legacy plan structures and strong compliance cultures. Healthcare shows high plan counts, possibly due to large institutional employers and unionized workforces.
...
Engineer a sector participant share variable.

```{r}
total_participants_all <- sum(plans$TOTAL_ACCBAL_PARTCP_EOY)

sector_participant_share <- plans %>%
  group_by(SECTOR_TITLE_SHORT) %>%
  summarise(total_participants = sum(TOTAL_ACCBAL_PARTCP_EOY, na.rm = TRUE)) %>%
...
```{r}
ggplot(sector_participant_share, aes(x = forcats::fct_reorder(SECTOR_TITLE_SHORT, 
                                                              total_participants, .desc = TRUE), 
                                     y = total_participants)) +
  geom_col(fill = "steelblue", color = "white") +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Share of Participants by Sector",
       x = "Sector",
       y = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5),
...
Low-coverage sectors like Agriculture, Mining, Education, and Public Administration are severely underrepresented. They may face structural barriers: seasonal labor, fragmented employment, or reliance on alternative retirement systems, e.g. public pensions are not captured in Form 5500.
...
plans %>%
  ...
  ggplot(aes(x = TOTAL_ASSETS_GROWTH_RATE, y = SECTOR_TITLE_SHORT)) +
...
Plot assets per participant excluding outliers over $500,000.

```{r}
plans %>% filter(ASSETS_PER_PARTCP < 500000) %>%
ggplot(aes(x = ASSETS_PER_PARTCP)) +
  geom_density(fill = "steelblue", alpha = 0.6) +
...
That sharp peak near the lower end, followed by a long right tail, suggests most plans cluster below $50,000 per participant with a mean of approximately $80,000.
...
Most plans are far below the mean: The log transformation exposes that the bulk of plans cluster near $59,000, not near six figures.
...
The "< $5K" tier shows a tight peak near $4,000, suggesting many plans cluster at very low adequacy. In contrast, the "$50K–$100K" tier is broader, indicating more heterogeneity in plan quality.

The "> $100K" tier is especially telling: a sharp peak just above $100,000 followed by a steep drop-off. That suggests a few high-performing plans dominate this tier, but most don’t go far beyond the threshold.
...
```{r}
ggplot(plans %>% filter(PARTCP_GROWTH_RATE <= 5), aes(x = PARTCP_GROWTH_RATE)) +
  geom_density(fill = "steelblue", alpha = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray40") +
...
```{r}
plans %>%
  filter(PARTCP_GROWTH_RATE <= 10) %>%
  ggplot(aes(x = PARTCP_GROWTH_RATE, y = SECTOR_TITLE_SHORT)) +
  geom_density_ridges(bandwidth = 0.05, fill = "steelblue", alpha = 0.6) +
...
```{r}
ggplot(plans, aes(x = PARTCP_GROWTH_RATE, y = reorder(SECTOR_TITLE_SHORT, PARTCP_GROWTH_RATE), fill = PARTCP_GROWTH_TIER)) +
  geom_col() +
...
plans %>%
  ...
  ggplot(aes(x = CONTRIB_PARTCP_GROWTH_RATE, y = SECTOR_TITLE_SHORT)) +
  geom_density_ridges(bandwidth = 0.05, fill = "steelblue", alpha = 0.6) +
...
```{r}
plans <- plans%>%
  mutate(CONTRIB_PARTCP_GROWTH_TIER= case_when(CONTRIB_PARTCP_GROWTH_RATE > 0.30 ~ "High Growth",
...
```{r}
ggplot(plans, aes(x = CONTRIB_PARTCP_GROWTH_RATE, y = reorder(SECTOR_TITLE_SHORT, CONTRIB_PARTCP_GROWTH_RATE), fill = CONTRIB_PARTCP_GROWTH_TIER)) +
  geom_col() +
  scale_fill_manual(values = c("High Growth" = "steelblue", "Negative Growth" = "lightsteelblue", "Typical" = "gray70")) +
  labs(title = "Contribution Growth Rate by Sector",
       x = "Growth Rate",
...
```{r}
plans <- plans %>%
  mutate(AVERAGE_CONTRIB_PER_PARTCP = round(TOTAL_CONTRIB_PARTCP_EOY / TOTAL_ACCBAL_PARTCP_EOY, 4))
```
...
plans <- plans %>%
  mutate(CONTRIB_PER_PARTCP_TIER = cut(AVERAGE_CONTRIB_PER_PARTCP,
                       breaks = c(-Inf, 0, 1000, 2000, 3000, Inf),
                       labels = c("Negative", "Low", "Moderate", "High", "Very High"),
                       right = FALSE),
         CONTRIB_PER_PARTCP_TIER = factor(CONTRIB_PER_PARTCP_TIER,
...
```{r}
plans <- plans%>%
  mutate(CONTRIB_EMPLR_GROWTH_TIER = case_when(CONTRIB_EMPLR_GROWTH_RATE > 0.30 ~ "High Growth",
...
```{r}
ggplot(plans, aes(x = CONTRIB_EMPLR_GROWTH_RATE, y = reorder(SECTOR_TITLE_SHORT, CONTRIB_EMPLR_GROWTH_RATE), fill = CONTRIB_EMPLR_GROWTH_TIER)) +
  geom_col() +
  scale_fill_manual(values = c("High Growth" = "steelblue", "Negative Growth" = "lightsteelblue", "Typical" = "gray70")) +
  labs(title = "Employer Contribution Growth Rate by Sector",
       x = "Growth Rate",
...
# Average Employer Contribution Per Participant (AVERAGE_EMPLR_CONTRIB_PER_PARTCP)

Implement average contribution per participant.

```{r}
plans <- plans %>%
  mutate(AVERAGE_EMPLR_CONTRIB_PER_PARTCP = round(TOTAL_CONTRIB_EMPLR_EOY / TOTAL_ACCBAL_PARTCP_EOY, 4))
```

Plot average contribution per participant by sector.

```{r}
summary_by_average_emplr_contribution <- plans %>%
  group_by(SECTOR_TITLE_SHORT) %>%
  summarise(AVERAGE_EMPLR_CONTRIB_PER_PARTCP = mean(AVERAGE_EMPLR_CONTRIB_PER_PARTCP))

ggplot(summary_by_average_emplr_contribution, aes(x = AVERAGE_EMPLR_CONTRIB_PER_PARTCP, 
                                            y = reorder(SECTOR_TITLE_SHORT,
                                                              AVERAGE_EMPLR_CONTRIB_PER_PARTCP))) +
  geom_col(fill = "steelblue") +
  geom_text(aes(label = scales::comma(round(AVERAGE_EMPLR_CONTRIB_PER_PARTCP))),
            hjust = -0.1, size = 3.5) +
  scale_x_continuous(labels = scales::label_comma()) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5),
        plot.title = element_text(size = 14, face = "bold")) +
  coord_cartesian(xlim = c(0, max(plans$AVERAGE_EMPLR_CONTRIB_PER_PARTCP) * 1.1))
```
...
```{r}
plans <- plans %>%
  mutate(ADEQUACY_SCORE = (ASSETS_PER_PARTCP > 55000) +
                          (TOTAL_ASSETS_GROWTH_RATE > 0.03) +
                          (AVERAGE_CONTRIB_PER_PARTCP > 1000) +
                          (LOAN_LEAKAGE_RATIO < 0.12),
