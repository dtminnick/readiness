...
Each condition is evaluated as a binary flag, and the adequacy score is the sum of conditions met. Plans meeting four or more criteria are classified as adequate.
...
Applying sector-specific quartiles below to generate a consistent set of tiers for testing model prediction.  This ensures tiers are derived relative to the sample, and not the adequacy cutoffs I used to create the response variable.
...
The final dataset includes `r format(nrow(plans), big.mark = ",", scientific = FALSE)` plans and `r format(ncol(plans), big.mark = ",", scientific = FALSE)` variables.
